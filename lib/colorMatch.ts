...
/// Instantiate once per Quick Match session (e.g. in a `useRef`) so
/// consecutive "Generate" taps avoid repeating the previous batch — see
/// `previousBatch` below, which mirrors the Dart service's instance-scoped
/// `_previousBatch` field. Intentionally not persisted anywhere.
export class ColorMatchGenerator {
  private previousBatch = new Set<string>();

  generate(options: QuickMatchOptions): GeneratedOutfit[] {
    const {
      items,
      count = 3,
        ...
        for (const bottom of bottoms) {
          for (const outer of outerwearOptions) {
            for (const shoe of shoeOptions) {
              combos.push(this.scoreCombo(top, bottom, outer, shoe));
            }
          }
        }
      }
    ...
    for (const avoidPreviousBatch of [true, false]) {
      if (chosen.length === resultCount) break;
      for (const combo of combos) {
        if (chosen.length === resultCount) break;
        const signature = combo.items.map((i) => i.id).join(",");
        if (chosenSignatures.has(signature)) continue;
        if (avoidPreviousBatch && this.previousBatch.has(signature)) continue;
    ...
    this.previousBatch = chosenSignatures;
